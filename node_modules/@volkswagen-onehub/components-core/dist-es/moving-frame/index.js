...
const StyledWrapper = styled.span.withConfig({ displayName: "StyledWrapper", componentId: "sc-16id0m5" }) `
	display: grid;
	align-items: center;
	width: 100%;
	...
	@media (min-width: ${Breakpoints.b2560}px) {
		grid-template-columns:
			calc(${Breakpoints.b2560 / 6}px + 50vw - var(--margin-size))
			max-content ${props => (props.hasClaim ? 'max-content ' : '')}auto;
	}
...
export const MovingFrame = (props) => {
    ...
    return (React.createElement(StyledWrapper, { hasClaim: Boolean(claim) },
        logo || React.createElement(Logo, { variant: "default" }),
        claim));
};
