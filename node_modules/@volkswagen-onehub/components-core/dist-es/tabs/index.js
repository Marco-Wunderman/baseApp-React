    ...
    render() {
        ...
        return (React.createElement(React.Fragment, null,
            React.createElement(StyledTabBar, { role: "tablist", ref: this.tabsRef, left: indicatorLeft, width: indicatorWidth, count: children.length }, children.map((tab, index) => (React.createElement(StyledTab, { role: "presentation", key: tab.key, id: `${idPrefix}-tab-${tab.key}`, "aria-controls": `${idPrefix}-content-${tab.key}`, index: index },
                React.createElement(StyledTabAnchor, { onClick: event => this.handleClick(event, index), tabIndex: activeIndex === index ? 0 : -1, ref: this.tabRefs[index], onKeyUp: this.handleKeyUp }, typeof tab.title === 'string' ? (React.createElement(Text, null, tab.title)) : (tab.title)))))),
...
const StyledTab = styled.li.withConfig({ displayName: "StyledTab", componentId: "sc-uzx9q3" }) `
	grid-area: ${props => String.fromCharCode(props.index + 65)};
`;
const tabBarIndicatorHeight = '2px';
const StyledTabBar = styled.ul.withConfig({ displayName: "StyledTabBar", componentId: "sc-1wtrart" }) `
	position: relative;
	display: grid;
...
`;
const StyledTabAnchor = styled.button.withConfig({ displayName: "StyledTabAnchor", componentId: "sc-vul809" }) `
