...
const StyledLabelText = theme_provider_1.styled.p.withConfig({ displayName: "StyledLabelText", componentId: "sc-136sgtd" }) `
	...
	top: 0;
`;
exports.FloatingStyledLabelText = theme_provider_1.styled.p.withConfig({ displayName: "FloatingStyledLabelText", componentId: "sc-gzhfid" }) `
...
`;
const StyledLabel = theme_provider_1.styled.label.withConfig({ displayName: "StyledLabel", componentId: "sc-pw4kdr" }) `
	display: block;
	width: 100%;
	position: relative;
	padding-top: ${props => props.theme.size.static250};
`;
const AdditionalInfo = theme_provider_1.styled.div.withConfig({ displayName: "AdditionalInfo", componentId: "sc-7kcxrm" }) `
	display: flex;
	flex-direction: row;
	justify-content: ${props => props.errorShouldBeVisible ? 'space-between' : 'flex-end'};
`;
const StyledErrorText = theme_provider_1.styled.p.withConfig({ displayName: "StyledErrorText", componentId: "sc-1gsseyr" }) `
	display: block;
	color: ${props => props.theme.interaction.recipe360.default};
	font-size: ${props => props.theme.textAppearances.copy0100.fontSize};
	margin: 0;
`;
const StyledCharacterCounterWrapper = theme_provider_1.styled.span.withConfig({ displayName: "StyledCharacterCounterWrapper", componentId: "sc-113ryio" }) `
	display: flex;
	justify-content: flex-end;
	color: ${utils_1.getStatusColor};
`;
const StyledIcon = theme_provider_1.styled.div.withConfig({ displayName: "StyledIcon", componentId: "sc-fo1bds" }) `
	position: absolute;
...
const TextLabel = ({ appearance, icon, isFloating, children }) => {
    if (isFloating) {
        return (React.createElement(exports.FloatingStyledLabelText, { appearance: appearance, hasIcon: Boolean(icon) }, children));
    }
...
exports.TextFieldWrapper = props => {
    const { additionalFeedback, appearance, children, errorText, label, maxLength, required, value, icon, isFloating } = props;
    const errorShouldBeVisible = Boolean((appearance === shared_1.TextAreaAppearance.Error ||
        appearance === shared_1.TextInputAppearance.Error) &&
        errorText);
